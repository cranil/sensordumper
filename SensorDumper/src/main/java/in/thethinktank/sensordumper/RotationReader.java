 ...
 */
public class RotationReader implements SensorReader, SensorEventListener {
    ...
    RotationReader(SensorManager sensorManager, File dumpFile) throws RuntimeException {
    ...
    RotationReader(SensorManager sensorManager, PrintWriter printWriter) throws RuntimeException {
        mSensorManager = sensorManager;
        mSensor = mSensorManager.getDefaultSensor(Sensor.TYPE_GYROSCOPE);
        mPrintWriter = printWriter ;
        if (mSensor == null){
    ...
    public void dump(long millisecondsStartAt, long millisecondsRate) {
        ...
        mTimer.scheduleAtFixedRate(new TimerTask() {
            @Override
            public void run() {
                synchronized (RotationReader.this) {
                    RotationReader.this.dump();
                }
            }
